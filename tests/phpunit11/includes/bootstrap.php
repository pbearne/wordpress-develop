...
if ( defined( 'WP_TESTS_CONFIG_FILE_PATH' ) ) {
	...
	$config_file_path = dirname( __DIR__, 2 );
	if ( ! file_exists( $config_file_path . '/wp-tests-config.php' ) ) {
		// Support the config file from the root of the develop repository.
		if ( basename( $config_file_path ) === 'phpunit11' && basename( dirname( $config_file_path ) ) === 'tests' ) {
...
require_once dirname( __DIR__, 2 ) . '/phpunit/includes/functions.php';
...
if ( version_compare( $phpunit_version, '11.0', '<' ) ) {
	printf(
		"Error: Looks like you're using PHPUnit %s. WordPress requires at least PHPUnit 11.0 for PHPUnit 11 tests." . PHP_EOL,
		$phpunit_version
	);
...
// Use the table prefix from the wp-tests-config.php file
global $table_prefix;
define( 'WP_TESTS_TABLE_PREFIX', $table_prefix );
define( 'DIR_TESTDATA', dirname( __DIR__, 2 ) . '/phpunit/data' );
define( 'DIR_TESTROOT', realpath( dirname( __DIR__, 2 ) . '/phpunit' ) );
...
if ( '1' !== getenv( 'WP_TESTS_SKIP_INSTALL' ) ) {
	...
	system( WP_PHP_BINARY . ' ' . escapeshellarg( dirname( __DIR__, 2 ) . '/phpunit/includes/install.php' ) . ' ' . escapeshellarg( $config_file_path ) . ' ' . $ms_tests . ' ' . $core_tests, $retval );
	if ( 0 !== $retval ) {
		exit( $retval );
	}
}
...
if ( $multisite ) {
	...
	echo 'Running as single site... To run multisite, use -c tests/phpunit11/multisite.xml' . PHP_EOL;
}
unset( $multisite );
...
require_once dirname( __DIR__, 2 ) . '/phpunit/includes/mock-mailer.php';
...
require dirname( __DIR__, 2 ) . '/phpunit/includes/phpunit-adapter-testcase.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/abstract-testcase.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-rest-api.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-rest-controller.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-rest-post-type-controller.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-xmlrpc.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-ajax.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-canonical.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/testcase-xml.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/exceptions.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/utils.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/spy-rest-server.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-rest-test-search-handler.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-rest-test-configurable-controller.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-fake-block-type.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-fake-hasher.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-sitemaps-test-provider.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-sitemaps-empty-test-provider.php';
require dirname( __DIR__, 2 ) . '/phpunit/includes/class-wp-sitemaps-large-test-provider.php';
